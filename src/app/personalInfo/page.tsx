...
import {tgtBscChain, tgtChain, config as wagmiConfig, bscWagmiConfig} from "@/config/wagmi";
...
export default function personalInfo() {
  ...
  const handleGetBscNFTList = async() => {
    ...
    let tokenIds:number[] = [];
    await readContract(bscWagmiConfig, {
      address: _addr,
      abi: NFT721ABI,
      functionName: 'walletOfOwner',
      args: [userInfo?.account],
      chainId: tgtBscChain?.id,
    })
    .then((res:any) => {
      console.log(res)
      res.forEach((item:any) => {
  ...
  const handleChangeOwnIdx = (tab: number) => {
    if(NFTlistLoading) {
      return;
    }
    setOwnIdx(tab);
  ...
  useEffect(() => {
    if(userInfo && NFTData && NFT1155Data) {
      if(ownIdx == 1) {
  ...
  }, [userInfo, NFTData, NFT1155Data, ownIdx])
              ...
              <div className="mt-14">
                ...
                <div className="relative">
                  <div className="flex justify-center py-3 mt-3">
                    <div className="flex bg-[#201E2A] rounded-[10px] px-2 md:px-6 py-2 justify-center space-x-3 md:space-x-15">
                      <button className={"w-30 text-[20px] text-white py-2 rounded-[10px] " + (ownIdx == 0 ? 'bg-[#FEBD32]' : 'hover:bg-[#FEBD32] bg-black/20')} onClick={() => handleChangeOwnIdx(0)}>{t('personalInfo.inGame')}</button>
                      <button className={"relative z-2 w-30 text-[20px] text-white py-2 rounded-[10px] " + (ownIdx == 1 ? 'bg-[#FEBD32] ' : 'hover:bg-[#FEBD32] bg-black/20')} onClick={() => handleChangeOwnIdx(1)}>
                        OP-BNB
                        <span className="text-[10px] text-zinc-200 text-center bg-[#2EBD85] w-[80px] md:w-[105px] py-1 rounded-lg absolute -top-4 right-0 translate-x-1/2 leading-[10px]">{t('personalInfo.confirmAirdropEntitlements')}</span>
                      </button>
                      <button className={"relative w-30 text-[20px] text-white py-2 rounded-[10px] " + (ownIdx == 2 ? 'bg-[#FEBD32] ' : 'hover:bg-[#FEBD32] bg-black/20')} onClick={() => handleChangeOwnIdx(2)}>
                        BSC
                        <span className="text-[10px] text-zinc-200 text-center bg-[#2EBD85] px-3 py-1 rounded-lg absolute top-0 -translate-y-1/2 right-0 translate-x-1/2 leading-[10px]">OG</span>
                      </button>
                    </div>
                  </div>
                ...
                <div className="">
                  ...
                  :
                    ...
                    :
                    <div className="text-center">
                      {ownIdx == 2 && <div className="w-full text-center text-[14px] text-[#8A84A3]">{t('personalInfo.bscEmptyHint')}</div>}
