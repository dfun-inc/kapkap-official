...
export default function Explore() {
  ...
  const handleNext = (type = "upcoming") => {
    if(type == "live") {
      const nextPage = (livePage + 1) < Math.ceil(appList['live'].length/pageLimit) ? livePage + 1 : Math.floor(appList['live'].length/pageLimit);
      liveSwiperRef.current?.slideTo(nextPage, 500);
      setLivePage(nextPage);
    }
    else if(type == "upcoming") {
      const nextPage = (upcomingPage + 1) < Math.ceil(appList['upcoming'].length/pageLimit) ? upcomingPage + 1 : Math.floor(appList['upcoming'].length/pageLimit);
  ...
  const handleGetIleData = async () => {
    if(appList?.live) {
      let temp:any = {};
      await Promise.all(appList['live'].map(async (item:any) => {
        if(item.process == 'launch') {
          await getILEData(item.id).then((res) => {
            const data = res?.data;
            if(data.status == 10000 && data.data.length) {
              temp[item.id] = data.data[0];
            }
          })
        }
      }));
  ...
  return (
    <main className="min-h-screen bg-[#121212]">
      <div className="max-w-[1920px] mx-auto relative">
        <Link className="fixed z-[2] right-6 md:right-9 top-[90%] md:top-1/2 -mt-3 animate__animated animate__fadeInUp rounded-full bg-[#6c4dfe] p-3 transform transition duration-300 ease-in-out hover:scale-110" href="/support">
        ...
        <div className="px-3 lg:px-18 2xl:px-24">
          <div className="py-12">
            <div className="pb-6">
              <div className="inline-block font-ethnocentric-rg text-white text-[22px] md:text-[30px] border-b border-[#FEBD32]">{t('explore.live')}</div>
              <div className="mt-3 text-[#8D73FF] md:text-[18px]">{t('explore.liveDesc')}</div>
            </div>
            {loading ? 
                                ...
                                <div className="flex flex-wrap">
                                  { appList['live'][i * pageLimit + j * pageLimit / 2 + k]?.tagArr.map((tagItem: string, tagIdx: number) => (
                                    <div key={tagIdx} className="mr-2">
                                      <Tag name={tagItem} index={tagIdx} />
                                    </div>  
                                  ))}
                                </div>
                                <div className="mt-[10px] text-[#A6A6A6] line-clamp-5 " dangerouslySetInnerHTML={{__html:  appList['live'][i * pageLimit + j * pageLimit / 2 + k]?.desc}}></div>
                              </div>
                            </Link>
                          ))}
                        </div>
              ...
              <div className="w-[300px] md:w-[400px] mx-auto rounded-full bg-black flex items-center mt-8 p-[5px]">
                <button className="rounded-full w-20 text-center bg-[#6c4dfe] py-1 text-center cursor-pointer hover:opacity-90" onClick={() => handlePrev('live')}>
                ...
                <button className="rounded-full w-20 text-center bg-[#6c4dfe] py-1 text-center cursor-pointer hover:opacity-90" onClick={() => handleNext('live')}>
          ...
          <div className="py-12">
            <div className="pb-6">
              <div className="inline-block font-ethnocentric-rg text-white text-[22px] md:text-[30px] border-b border-[#FEBD32]">{t('explore.upcoming')}</div>
              <div className="mt-3 text-[#8D73FF] md:text-[18px]">{t('explore.upcomingDesc')}</div>
            </div>
            {loading ? 
              Array.from({ length: 2 }).map((_, i) => (
                <div key={i} className="flex justify-between flex-wrap gap-8 mt-8">
             ...
             : 
                          ...
                          {Array.from({ length: pageLimit / 2 }).map((_, k) => (
                            ...
                            <Link href={'/project?id=' + appList['upcoming'][i * pageLimit + j * pageLimit / 2 + k]?.id} key={k} className="group block flex-1 rounded-[20px] overflow-hidden bg-black hover:bg-[#201e2a] cursor-pointer">
                              ...
                              <div className="px-[20px] py-5">
                                ...
                                <div className="mt-[10px] text-[#A6A6A6] line-clamp-5 " dangerouslySetInnerHTML={{__html:  appList['upcoming'][i * pageLimit + j * pageLimit / 2 + k]?.desc}}></div>
                              </div>
                            </Link>
                          ))}
                        </div>
              ...
              <div className="w-[300px] md:w-[400px] mx-auto rounded-full bg-black flex items-center mt-8 p-[5px]">
                <button className="rounded-full w-20 text-center bg-[#6c4dfe] py-1 text-center cursor-pointer hover:opacity-90" onClick={() => handlePrev('upcoming')}>
                  <svg className="mx-auto" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24"><path d="M358.997 512l311.168-311.168a42.667 42.667 0 1 0-60.33-60.33L268.5 481.834a42.667 42.667 0 0 0 0 60.33L609.835 883.5a42.667 42.667 0 0 0 60.33-60.331L358.997 512z" fill="#ffffff"></path></svg>
                </button>
                <div className="flex-1 text-center text-[18px]">{upcomingPage + 1}/{Math.ceil(appList['upcoming'].length / pageLimit)}</div>
                <button className="rounded-full w-20 text-center bg-[#6c4dfe] py-1 text-center cursor-pointer hover:opacity-90" onClick={() => handleNext('upcoming')}>
                  <svg className="mx-auto" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" width="24" height="24"><path d="M665.003 512L353.835 200.832a42.667 42.667 0 0 1 60.33-60.33L755.5 481.834a42.667 42.667 0 0 1 0 60.33L414.165 883.5a42.667 42.667 0 1 1-60.33-60.331L665.003 512z" p-id="3110" fill="#ffffff"></path></svg>
                </button>
              </div>
              </>
